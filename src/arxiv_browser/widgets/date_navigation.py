"""Date-history navigation widget and responsive layout helpers."""
...
def _navigation_label(content: str, classes: str, item_id: str, tooltip: str) -> Label:
...
class DateNavigator(Horizontal):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield _navigation_label("<", "date-nav-arrow", "date-nav-prev", "Older (])")
        yield _navigation_label(">", "date-nav-arrow", "date-nav-next", "Newer ([)")
    ...
    def _build_desired_items(
        ...
        for i in range(start, end):
            current_date, _ = history_files[i]
            count = self._get_paper_count(i) if label_mode == DATE_NAV_LABEL_WITH_COUNTS else None
            label_text = _format_date_nav_label(current_date, count=count, mode=label_mode)
            tooltip = f"Jump to {current_date.isoformat()}"
...
__all__ = ["DATE_NAV_WINDOW_SIZE", "DateNavigator"]
