...
def _config_to_dict(config: UserConfig) -> dict[str, Any]:
    ...
    return {
        ...
        "judge_paper_limit": config.judge_paper_limit,
        "judge_pairwise_top_k": config.judge_pairwise_top_k,
...
def _dict_to_config(data: dict[str, Any]) -> UserConfig:
    ...
    return UserConfig(
        ...
        **_deadline_config.from_dict(data),
        ...
        judge_paper_limit=max(1, min(500, _safe_get(data, "judge_paper_limit", 50, int))),
        judge_pairwise_top_k=max(0, min(32, _safe_get(data, "judge_pairwise_top_k", 0, int))),
